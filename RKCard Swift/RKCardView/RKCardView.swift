...
class RKCardView: UIVisualEffectView {
...
//        super.init(frame: frame)
    ...
    let CORNER_RATIO:CGFloat = 0.015
    let CP_RATIO:CGFloat = 0.38
    let PP_RATIO:CGFloat = 0.247
    let PP_X_RATIO:CGFloat = 0.03
    let PP_Y_RATIO:CGFloat = 0.213
    let PP_BUFF:CGFloat = 3
    let LABEL_Y_RATIO:CGFloat = 0.012
    
    
    var delegate:RKCardViewDelegate? = nil
...
//        visualEffectView.alpha = 0
    ...
    // Setup the label
        ...
        let titleLabel:UILabel = UILabel(frame: CGRectMake(titleLabelX, cp_mask.frame.size.height + 7, self.frame.size.width - titleLabelX, 26))
    ...
    // Register touch events on the profile imate
        ...
        coverImageView.addGestureRecognizer(tapGestureProfile)
